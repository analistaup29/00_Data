...
siagie_2020 <- read_xlsx(filepath(bases_crudas, "02_OSEE", "01_siagie", "siagie_2017_2020.xlsx"), sheet = "2020", col_names = TRUE ) %>% clean_names()
...
siagie_cod_mod <- siagie_2020 %>%
  ...
  summarize(taprobado_siagie = sum(taprobado_siagie),
            ...
            requiere_recuperacion_pedagogica = sum(requiere_recuperacion_pedagogica),
...
write_dta(siagie_cod_mod, path = filepath(bases_crudas, "02_OSEE", "01_siagie", "siagie_cod_mod.dta"))
write_rds(siagie_cod_mod, path = filepath(bases_crudas, "02_OSEE", "01_siagie", "siagie_cod_mod.rds"))
