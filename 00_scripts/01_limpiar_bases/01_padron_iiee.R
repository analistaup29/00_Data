...
padron_iiee<-padron_iiee %>%
  mutate_if(is.character, 
            function(col) iconv(col, from = "IBM850", to = "utf-8"))
...
padron_iiee$d_niv_mod <- labelled(padron_iiee$d_niv_mod, label="Nivel / Modalidad", labels = d_niv_mod_levels)
padron_iiee$d_niv_mod <- as_factor(padron_iiee$d_niv_mod, levels = "values")

padron_iiee$d_forma <- labelled(padron_iiee$d_forma, label="Forma de atención", labels = d_forma_levels)
padron_iiee$d_forma <- as_factor(padron_iiee$d_forma, levels ="values")

padron_iiee$d_cod_car <- labelled(padron_iiee$d_cod_car, label="Caracteristica (Censo educativo 2020)", labels = d_cod_car_levels)
padron_iiee$d_cod_car <- as_factor(padron_iiee$d_cod_car, levels ="values")

padron_iiee$d_tipssexo <- labelled(padron_iiee$d_tipssexo, label="Género de los alumnos", labels = d_tipssexo_levels)
padron_iiee$d_tipssexo <- as_factor(padron_iiee$d_tipssexo, levels ="values")

padron_iiee$d_gestion <- labelled(padron_iiee$d_gestion, label="Gestión del servicio educativo", labels = d_gestion_levels)
padron_iiee$d_gestion <- as_factor(padron_iiee$d_gestion, levels ="values")

padron_iiee$d_ges_dep <- labelled(padron_iiee$d_ges_dep, label="Gestión / Dependencia", labels = d_ges_dep_levels)
padron_iiee$d_ges_dep <- as_factor(padron_iiee$d_ges_dep, levels ="values")

padron_iiee$dareacenso <- labelled(padron_iiee$dareacenso, label="Área geográfica (500 Habitantes)", labels = dareacenso_levels)
padron_iiee$dareacenso <- as_factor(padron_iiee$dareacenso, levels ="values")

padron_iiee$d_tipoprog <- labelled(padron_iiee$d_tipoprog, label="Tipo de programa", labels = d_tipoprog_levels)
padron_iiee$d_tipoprog <- as_factor(padron_iiee$d_tipoprog, levels ="values")

padron_iiee$d_cod_tur <- labelled(padron_iiee$d_cod_tur, label="Turno de atención", labels = d_cod_tur_levels)
padron_iiee$d_cod_tur <- as_factor(padron_iiee$d_cod_tur, levels ="values")

padron_iiee$d_estado <- labelled(padron_iiee$d_estado, label="Estado del servicio educativo", labels = d_estado_levels)
padron_iiee$d_estado <- as_factor(padron_iiee$d_estado, levels ="values")

padron_iiee$d_fte_dato <- labelled(padron_iiee$d_fte_dato, label="Tipo de la fuente de datos Censo 2020", labels = d_fte_dato_levels)
padron_iiee$d_fte_dato <- as_factor(padron_iiee$d_fte_dato, levels ="values")
